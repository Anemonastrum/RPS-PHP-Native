    ...
    <div class="d-flex">
        ...
        if (isset($_GET['kode_matkul'])) {
            ...
            if ($result && $row = mysqli_fetch_assoc($result)) {
                ...
                echo "Record not found in matkuldb.";
                // Handle this case as needed
            }

            // Fetch data from the 'materidb' table based on the provided 'matkul'
        ...
        <div id="content" class="content-t flex-grow-1 p-4">
            <div style="display: flex; align-items: center;">
                <a class="ms-2 fw-bold btn btn-dark rounded-circle me-1" href="list.php"><i class='fas fa-arrow-left'></i></a>
                <?php echo '<h3 class="px-2 pt-3 pb-2 fw-bold">View RPS'; ?> <?php echo $record_id; ?> <?php echo '</h3>'; ?>
            </div>
                        ...
                        <?php
                            if (isset($result2)) {
                                while ($row2 = mysqli_fetch_assoc($result2)) {
                                    for ($i = 1; $i <= 14; $i++) {
                                        // Check if both jdmateri and materi are not null
                                        if ($row2['jdmateri' . $i] !== "") {
                                            echo '<tr>';
                                            echo '<td>' . $i . '</td>';
                                            echo '<td>' . $row2['jdmateri' . $i] . '</td>';
                                            echo '<td>' . $row2['materi' . $i] . '</td>';
                                            echo "<td class='act'>";
                                            echo "<a href='edit.php?kode_matkul=" . $record_id . "'><button class='btn btn-dark btn-sm me-1'><i class='fas fa-edit'></i></button></a>";
                                            echo "<a href='assets/php/delMateri.php?kode_matkul=" . $record_id . "&materi=" . $i . "'><button class='btn btn-dark btn-sm me-1'><i class='fas fa-trash'></i></button></a>";
                                            echo "</td>";
                                            echo '</tr>';
                                        }
                                    }
